<#
.SYNOPSIS
    This PowerShell script ensures IPv6 source routing is configured to the highest protection.
.NOTES
...
if (-not (Get-ItemProperty -Path $RegistryPath -Name $ValueName -ErrorAction SilentlyContinue)) {
    ...
    $CurrentValue = (Get-ItemProperty -Path $RegistryPath -Name $ValueName).DisableIpSourceRouting

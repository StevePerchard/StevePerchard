<#
.SYNOPSIS
    This PowerShell script ensures autoplay is turned off for non-volume devices.
.NOTES
...
.USAGE
    ...
    Example syntax:
    PS C:\> .\(STIG-ID-WN10-CC-000180).ps1 
#>
...
if (-not (Get-ItemProperty -Path $RegistryPath -Name $ValueName -ErrorAction SilentlyContinue)) {
    ...
    $CurrentValue = (Get-ItemProperty -Path $RegistryPath -Name $ValueName).NoAutoplayfornonVolume
